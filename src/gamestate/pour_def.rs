...
impl<'a, T: GameState + Clone> ValidPour<'a, T> {
    ...
    pub fn apply(&self) -> T {
        ...
        let (left, right) = new_game_state.get_mut_bottles().split_at_mut(
            if split_on_source {
    ...
    pub fn get_source_gamestate(&self) -> &T {
    ...
    pub fn get_source(&self) -> &T::BottleT {
        self.source_gamestate
            .get_bottles()
    ...
    pub fn get_dest(&self) -> &T::BottleT {
        self.source_gamestate
            .get_bottles()
...
impl<'a, T: GameState + Clone> Iterator for ValidPourIter<'a, T> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        for from_index in self.current_from_index..self.source_gamestate.get_bottles().len() {
            for to_index in self.current_to_index..self.source_gamestate.get_bottles().len() {
